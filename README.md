...
- **三段敏捷評分**：提供 `💥 忘記 (Again)`、`🤔 不熟 (Hard)`、`💡 掌握 (Good)` 三階人體工學按鈕（精簡決策維度，去除 Easy 以降低單手操作之認知負擔），卡片翻面即時預覽下次複習間隔（例如 `< 1分`、`1天`、`3天`）。

### 3. 🎴 雙軌手勢互動引擎 & 慣用手人體工學 (v1.3.4 Innovations)
- **正面雙向靈動手勢 & 唯讀回看上一詞 (Front Gestures & Rewind Peek)**：
  - 👉 **點擊或向右滑 (➔)**：3D 原地旋轉翻面查看詳解，符合單手向右輕撥之自然觸控直覺。
  - 👈 **向左滑 (←)**：流暢倒帶切換回「上一張單字卡片」，供隨時回看複習（唯讀模式，絕不破壞 FSRS 評分與排程數據）。
  - ↺ **唯讀速覽保護**：回看模式頂部提示「剛剛看過的單字」，底部提供「➔ 返回當前題目」快捷膠囊與右滑瞬回，防重複評分。
